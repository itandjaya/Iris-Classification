...
def one_hot(y, y_max = 2+1):
    arr     =   [];
    

    for y_val in y:
        arr.append( np.eye(y_max)[y_val] );
...
class NN_Model:
    ...
    def __init__(self, X = None, y = None, num_layers = 4, arr_layer_sizes = [400,50,12,10]):
        ...
        self.var_lambda     = 0;
        self.var_alpha      = 0.5;
        self.learning_iter  = 50000;            ##  With current weights initialization,
                                                ##  the weights should converge at iter ~= 2.5k.
        self.J_Cost_values  = [];

        self.NUM_LAYERS     = num_layers;       ## 3-layers NN: input layer, inter-layer1, 
                                                ##              output-layer.

        m, n   =    np.shape(X);
        self.y = y;
        self.X      =   np.concatenate(   ( np.ones( (m,1) ), X ), 1); # add bias weight 1.
    ...
    def one_hot(self, y, y_max = 2):
        return one_hot(y, y_max);
